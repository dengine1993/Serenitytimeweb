...
serve(async (req) => {
  ...
  try {
    ...
    // SECURITY: ignore client-supplied price; always use catalog price
    let amount = productDef.amount.value;
    let appliedDiscount: { kind: string; pct: number } | null = null;

    // Apply referral discount for subscription products (server-side only)
    if (productDef.entitlement.kind === 'subscription') {
      ...
      if (referral) {
        ...
        if (!firstPayment) {
          ...
          appliedDiscount = { kind: 'referral_first_payment', pct: discountPct };
          console.log(`Applied ${discountPct}% referral discount: ${productDef.amount.value} -> ${amount}`);
        }
      }
    }
    ...
    // Create payment record (with discount snapshot for webhook validation)
    const { data: payment, error: paymentError } = await supabaseAdmin
      ...
      .insert({
        ...
        status: 'pending',
        meta: {
          catalog_amount: productDef.amount.value,
    ...
    const yookassaPayload: Record<string, unknown> = {
      amount: {
      ...
      metadata,
      // Для подписочных продуктов сохраняем способ оплаты, чтобы edge-функция
      // subscription-charge-recurrent могла автоматически продлевать подписку
      // через payment_method_id.
      ...(productDef.entitlement.kind === 'subscription'
